...
(function(root, factory) {
    ...
    var Transport = function() {
        ...
        _.mixin(Transport.prototype, {
            _fingerprint: function fingerprint(o) {
                o = o || {};
                return o.url + o.type + jQuery.param(o.data || {});
            },
            _get: function(o, cb) {
    ...
    var oParser = function() {
        ...
        return function parse(o) {
            ...
            !o.datumTokenizer && jQuery.error("datumTokenizer is required");
            !o.queryTokenizer && jQuery.error("queryTokenizer is required");
            sorter = o.sorter;
        ...
        function parsePrefetch(o) {
            ...
            o = _.mixin(defaults, o);
            !o.url && jQuery.error("prefetch requires url to be set");
            o.transform = o.filter || o.transform;
            o.cacheKey = o.cacheKey || o.url;
            o.thumbprint = VERSION + o.thumbprint;
            o.transport = o.transport ? callbackToDeferred(o.transport) : jQuery.ajax;
            return o;
        }
        function parseRemote(o) {
            ...
            o = _.mixin(defaults, o);
            !o.url && jQuery.error("remote requires url to be set");
            o.transform = o.filter || o.transform;
...
(function(root, factory) {
    ...
    var EventBus = function() {
        ...
        _.mixin(EventBus.prototype, {
            _trigger: function(type, args) {
                var $e = jQuery.Event(namespace + type);
    ...
    var Input = function() {
        ...
        function Input(o, www) {
            ...
            jQuery(www.menu).attr("id", this.$input.attr("id") + "_listbox");
        ...
        _.mixin(Input.prototype, EventEmitter, {
            ...
            destroy: function destroy() {
                ...
                this.$hint = this.$input = this.$overflowHelper = jQuery("<div>");
            }
        });
        return Input;
    ...
    var Dataset = function() {
        ...
        function Dataset(o, www) {
            ...
            this.$el = jQuery(o.node).attr("role", "presentation").addClass(this.classes.dataset).addClass(this.classes.dataset + "-" + this.name);
        }
        ...
        _.mixin(Dataset.prototype, EventEmitter, {
            ...
            _getSuggestionsFragment: function getSuggestionsFragment(query, suggestions) {
                ...
                _.each(suggestions, function getSuggestionNode(suggestion) {
                    ...
                    $el = jQuery(that.templates.suggestion(context)).data(keys.dataset, that.name).data(keys.obj, suggestion).data(keys.val, that.displayFn(suggestion)).addClass(that.classes.suggestion + " " + that.classes.selectable);
        ...
        function getTemplates(templates, displayFn) {
            ...
            function suggestionTemplate(context) {
                return jQuery('<div role="option">').attr("id", _.guid()).text(displayFn(context));
            }
        }
    ...
    var Menu = function() {
        ...
        function Menu(o, www) {
            ...
            o = o || {};
            if (!o.node) {
                jQuery.error("node is required");
            }
            www.mixin(this);
            this.$node = jQuery(o.node);
            this.query = null;
            this.datasets = _.map(o.datasets, initializeDataset);
            function initializeDataset(oDataset) {
                var node = that.$node.find(oDataset.node).first();
                oDataset.node = node.length ? node : jQuery("<div>").appendTo(that.$node);
                return new Dataset(oDataset, www);
            }
        }
        _.mixin(Menu.prototype, EventEmitter, {
            _onSelectableClick: function onSelectableClick($e) {
                this.trigger("selectableClicked", jQuery($e.currentTarget));
            },
    ...
    var Typeahead = function() {
        ...
        function Typeahead(o, www) {
            ...
            if (!o.menu) {
                jQuery.error("missing menu");
            }
            if (!o.eventBus) {
                jQuery.error("missing event bus");
            }
            www.mixin(this);
        ...
        _.mixin(Typeahead.prototype, {
            _hacks: function hacks() {
                var $input, $menu;
                $input = this.input.$input || jQuery("<div>");
                $menu = this.menu.$node || jQuery("<div>");
        ...
        methods = {
            initialize: function initialize(o, datasets) {
                ...
                function attach() {
                    ...
                    $input = jQuery(this);
                    $wrapper = jQuery(www.html.wrapper);
                    $hint = $elOrNull(o.hint);
                    $menu = $elOrNull(o.menu);
                    defaultHint = o.hint !== false && !$hint;
                    defaultMenu = o.menu !== false && !$menu;
                    defaultHint && ($hint = buildHintFromInput($input, www));
                    defaultMenu && ($menu = jQuery(www.html.menu).css(www.css.menu));
                    $hint && $hint.val("");
            ...
            select: function select(el) {
                var success = false, $el = jQuery(el);
                ttEach(this.first(), function(t) {
                    success = t.select($el);
                });
                return success;
...
jQuery('[data-docs-code]').each(function(index, value) {
  var copyBtnId = 'copy-btn-' + index.toString();
  var $button = jQuery('<button class="docs-code-copy" id="' + copyBtnId + '">Copy</button>');

  var text = jQuery(this).find('code').text()
    .replace('&lt;', '<')
    .replace('&gt;', '>');
...
$ks.find('[data-ks-codepen]').each(function() {
  var $codepen = jQuery(this);
  var $code = $codepen.next('[data-docs-code]');

  $link = jQuery('<a class="docs-code-toggle">Toggle Code</a>');
...
var source = {
  ...
  source: function(query, sync, async) {
    ...
    jQuery.getJSON('./data/search.json', function(data, status) {
...
var $h2s = jQuery('.docs-component h2');
var $toc = jQuery('[data-docs-toc]');
...
//var $container = jQuery('.docs-component');
//var $TOC = jQuery('#docsToc');
//var $TOCparent = $TOC.parent();
//
//if($TOC.is('*')) {
//  var fixedTop = $container.offset().top;
//  var innerHeight = 0;
//  $container.children().each(function() {innerHeight += jQuery(this).height()});
...
//    }
//  };
//  jQuery(document).on('load scroll', handler);
...
(function() {
  var prev = 0;
  var $window = jQuery(window);
  var nav = jQuery('.docs-sticky-top-bar');
  ...
  jQuery('#inline-search').on('on.zf.toggler', function() {
    jQuery(this).find('input').focus();
    jQuery('.search-button').addClass('search-open');
  });
  jQuery('#inline-search').on('off.zf.toggler', function() {
    jQuery(this).find('input').blur();
    jQuery('.search-button').removeClass('search-open');
  });
})();
...
if (jQuery('#main-video').is('*')) {
  var $videoOuter = jQuery('#subpage-intro-video');
  var $videoInner = $videoOuter.find('.docs-video-inner');
  var $videoOverlay = $videoOuter.find('.video-overlay');
  var videoId = jQuery('#main-video').data().video;
  ...
  function onYouTubeIframeAPIReady() {
    ...
    jQuery('[data-linkable-video]').each(function() {
      var $vid= jQuery(this);
      var id = this.id;
    ...
    if(QueryString.video == videoId) {
      ...
      for(var i = 0; i < embeddedPlayers.length; i++) {
        if(QueryString.video == embeddedPlayers[i].video) {
          jQuery(window).scrollTop(jQuery('#' + embeddedPlayers[i].id).offset().top - 200);
  ...
  jQuery(window).on("scroll", function() {
    ...
    var myScrollPosition = jQuery(this).scrollTop();
  ...
  jQuery('[data-close-video]').on('click', function() {
    mainPlayer.stopVideo();
    $videoInner.removeClass('autostick').removeClass('expanded');
    $videoOverlay.removeClass('expanded');
  });

  jQuery('[data-expand-contract-video]').on('click', function() {
    $videoInner.toggleClass('expanded');
    $videoOverlay.toggleClass('expanded');
  });

  var getSeconds = function(link) {
    var time = jQuery(link).data().openVideo;
  ...
  var href = jQuery('#docs-mobile-video-link').attr('href');
  jQuery('[data-open-video]').each(function() {
    var seconds = getSeconds(this);
    this.href = href + '&t=' + seconds;
    this.target = '_blank';
  });

  jQuery('[data-open-video]').on('click', function(e) {
...
jQuery(function() {
  ...
  var topic = jQuery('h1.docs-page-title').text();
...
function addBuildingBlocks(data) {
  ...
  jQuery('[data-building-blocks]').each(function() {
    jQuery(this).html(html);
  });

  if (jQuery('[data-building-blocks] .docs-bb').length === 0) {
    jQuery('.docs-building-blocks').hide(0);
  }
}
...
jQuery('[data-docs-example-ofc]').click(function() {
  jQuery('#offCanvasLeft').toggleClass('reveal-for-large');
  jQuery('.sticky').foundation('_calc', true);
});

jQuery('[data-docs-example-series]').click(function() {
  jQuery('#series-example').addClass('is-animating');
});

var $transitionDemo = jQuery('.docs-transition-demo');
jQuery('.docs-transitions').change(function() {
  var value = jQuery(this).val();
  var method = value.match('-in') ? 'animateIn' : 'animateOut';
console.log(value, method, MotionUI);
  MotionUI[method]($transitionDemo, value, function() {
    $transitionDemo.show();
  });
});
var stickyMag = jQuery('#sticky-magellan');
...
!function() {
var loaded = false;
jQuery('#docs-example-interchange').on('replaced.zf.interchange', function() {
  if(Foundation.MediaQuery.atLeast('large')){
    if(!loaded){
      jQuery.getScript("https://maps.googleapis.com/maps/api/js?key=AIzaSyBOVwxUM9akvFrSWmmb2iKc7Fe0vjRBY7c&sensor=false&callback=initializeMaps")
      .done(function(){
        loaded = true;
      });
    }else{
...
jQuery(function() {
  // Equalizer test
  var counter = 0;
  jQuery('#test-eq').on('postEqualized.zf.Equalizer', function() {
    counter++;
    console.log(counter);
  });
  jQuery('#pokemonRed').on('invalid.fndtn.abide', function(e, data) {
    console.log(data);
  });
});

jQuery(function() {
  jQuery('[data-docs-version]').text('v' + Foundation.version);
});
...
jQuery('[data-expand-all]').on('click', function() {
  var $a = jQuery(this);
...
if(localStorage.getItem(ACCORDION_KEY) === 'true') {
  expandAccordion(jQuery('[data-expand-all]'));
} else {
  jQuery('[data-expand-all]').text('Expand');
}
